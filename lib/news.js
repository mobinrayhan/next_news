...
export async function getLatestNews() {
  const latestNews = DUMMY_NEWS.sort(
...
export async function getAvailableNewsYears() {
  const years = DUMMY_NEWS.map((news) => news.date.split("-")[0])
    .sort((a, b) => b - a)
    .reduce((acc, year) => (acc.includes(year) ? acc : [...acc, year]), []);
...
export function getAvailableNewsMonths(year) {
  const months = DUMMY_NEWS.filter((news) => news.date.split("-")[0] === year)
    .map((month) => month.date.split("-")[1].replace("0", ""))
    .sort((a, b) => b - a);
...
export async function getNewsForYear(year) {
  const news = DUMMY_NEWS.filter((news) => news.date.split("-")[0] === year);
...
export async function getNewsForYearAndMonth(year, month) {
  const news = DUMMY_NEWS.filter(
    (news) =>
      news.date.split("-")[0] === year &&
      news.date.split("-")[1].replace("0", "") === month
  );
